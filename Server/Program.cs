...
app.MapGet("/posts/all", handler: async (MiniCourseDbContext dbContext) =>
{
    List<Post> allPosts = await dbContext.Posts.ToListAsync();
    return allPosts;
});

app.MapGet("/posts/by-id/{postId}", handler: async (int postId, MiniCourseDbContext dbContext) =>
{
...
app.MapPost("/posts/create", handler: async (Post postToCreate, MiniCourseDbContext dbContext) =>
{
...
app.MapPut("/posts/update/{postId}", handler: async (int postId, Post updatedPost, MiniCourseDbContext dbContext) =>
{
...
app.MapDelete("/posts/delete/{postId}", handler: async (int postId, MiniCourseDbContext dbContext) =>
{
